...
async function bootstrapSshConnectionInner(profile, sshConfig, reuseToken, source, metadata, fingerprint, lease) {
  const scope = sshScopeKey(profile)
  const backendRole = normalizeDesktopBackendRole(metadata.backendRole)
...
async function ensureRegistryBackend(connectionId, profile, managedUpdateCorrelation = '') {
  ...
  if (
    ...
    desktopRegistryUsesSharedCronOwner(primaryRoute.source, primaryRoute.connectionId, id)
  ) {
    const primary = await ensureBackend(primaryProfile)
    ...
    return {
      ...
      profile: profileKey,
      connectionId: id,
      remoteProfile: primary.remoteProfile || primaryProfile,
      sharedPrimary: true,
      sharedRemote: true
    }
  }
  ...
  const key = backendScopeKey(id, ownershipProfile)
  const existing = backendPool.get(key)
  ...
  if (existing) {
    ...
    return registryDispatchRevalidation
      .run(connectionPromise, (): Promise<any> =>
        ensureHealthyPooledRemoteBackendForDispatch({
          connectionPromise,
  ...
  const entry = {
    ...
    token: null,
    connectionPromise: null,
    cronOwner: source.kind === 'ssh' && desktopRegistryBackendRole(ownershipProfile) === 'primary',
    lastActiveAt: Date.now(),
    remoteBaseUrl: null
  }
  ...
  return forRequestedProfile(await entry.connectionPromise)
}
...
async function connectRegistryBackend(
  ...
  if (source.kind === 'ssh') {
    ...
    const connection = await bootstrapSshConnection(
      ...
      {
        backendRole: desktopRegistryBackendRole(profileKey),
        managedScope: 'pool',
        managedUpdateCorrelation,
        poolKey: key,
        registryConnectionId: source.id
